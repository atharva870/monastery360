...
const MONASTERIES = [
  {
    ...
    lat: 27.3177,
    lon: 88.619,
    text: "You are near Rumtek Monastery, the seat of the Karmapa lineage. The main shrine features intricate woodwork and vibrant murals.",
  },
  {
    name: "Pemayangtse Monastery",
    lat: 27.3019,
    lon: 88.2345,
    text: "Pemayangtse Monastery overlooks Pelling. Do not miss the wooden masterpiece 'Zangdok Palri' inside the top floor museum.",
  },
];
  ...
  useEffect(() => {
    ...
    const watch = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        const nearest = MONASTERIES.map((m) => ({
          m,
          d: haversine(here, m),
        })).sort((a, b) => a.d - b.d)[0];
    ...
    <div className="container py-12">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-prose text-foreground/70">
            Location-based narration using your device’s GPS and the browser’s
            speech engine. Works offline when cached.
          </p>
        </div>
      ...
      <section className="mt-8 rounded-xl border p-4">
        ...
        <ul className="mt-2 list-disc pl-5 text-sm text-foreground/70">
          <li>
            We compare your GPS position to nearby monasteries (within 1 km) and
            play a short narration.
          </li>
          <li>No personal data is uploaded; all processing runs on-device.</li>
          <li>
            Offline mode caches pages and images so guides continue to work with
